...
output_WPS_v3 = snakemake.output["output_WPS_v3"]
...
def normalize_sample(path_a: str, path_b0: str, path_b1: str, path_b2: str, path_b3: str, path_b4: str, method: str="FoldChange"):
    ...
    if method == "FoldChange":
        ...
        for i in range(0,sample_a.shape[1]-1):
            ...
            sample.iloc[:,i] = (sample_a.iloc[:,i].values+1) / (0.2*(sample_b0.values+sample_b1.values+sample_b2.values+sample_b3.values+sample_b4.values)+1) # pseudo count: 0.1
    else:
        #print("WPS using Delta")  
        for i in range(0,sample_a.shape[1]-1):
            #sys.stderr.write("using Delta normalize...\n")
            sample.iloc[:,i] = sample_a.iloc[:,i].values - 0.2*(sample_b0.values+sample_b1.values+sample_b2.values+sample_b3.values+sample_b4.values)
    return sample.round(4)
#        sample = sample_a / stats.trim_mean(sample_b, 0.1) # + 0.1 # pseudo count

normalized_WPS = normalize_sample(path_a=target_WPS, path_b0=background_WPS0, path_b1=background_WPS1, path_b2=background_WPS2, path_b3=background_WPS3, path_b4=background_WPS4, method="Delta")
normalized_WPS_v2 = normalize_sample(path_a=target_WPS_v2, path_b0=background_WPS0_v2, path_b1=background_WPS1_v2, path_b2=background_WPS2_v2, path_b3=background_WPS3_v2, path_b4=background_WPS4_v2, method="FoldChange")
normalized_WPS_v3 = normalize_sample(path_a=target_WPS, path_b0=background_COV0, path_b1=background_COV1, path_b2=background_COV2, path_b3=background_COV3, path_b4=background_COV4, method="FoldChange")
normalized_COV = normalize_sample(path_a=target_COV, path_b0=background_COV0, path_b1=background_COV1, path_b2=background_COV2, path_b3=background_COV3, path_b4=background_COV4, method="FoldChange")
normalized_STARTS = normalize_sample(path_a=target_STARTS, path_b0=background_STARTS0, path_b1=background_STARTS1, path_b2=background_STARTS2, path_b3=background_STARTS3, path_b4=background_STARTS4, method="FoldChange")
...
if normalized_WPS_v3.index.is_object():
    normalized_WPS_v3.to_csv(output_WPS_v3, sep="\t",header=None,compression='gzip')
else:
    normalized_WPS_v3.to_csv(output_WPS_v3, sep="\t",header=None, index=None,compression='gzip')
