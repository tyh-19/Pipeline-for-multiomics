...
#STARTS = snakemake.input["STARTS"]
#STARTS_refs = snakemake.input["STARTS_ref"]
...
#STARTS_back = snakemake.input["STARTS_back"]
#STARTS_back_refs = snakemake.input["STARTS_back_ref"]
...
flank_edge = 500  # total: tss200+500*2=1200 bp
...
# av_STARTS = pd.DataFrame(add_sample(STARTS, STARTS_back,overlay_mode,smoothing,rolling, background_norm))
# sys.stderr.write("STARTS [%s]: %s %s\n"%(sample_ID, STARTS, STARTS_back))
# av_STARTS.columns = av_STARTS.columns.astype(str)
# av_STARTS.columns.values[-1] = sample_ID
# for (ref_ID, STARTS_ref, STARTS_back_ref) in zip(ref_IDs, STARTS_refs, STARTS_back_refs):
#     sys.stderr.write("STARTS [%s]: %s %s\n"%(ref_ID, STARTS_ref, STARTS_back_ref))
#     av_STARTS[ref_ID] = add_sample(STARTS_ref, STARTS_back_ref,overlay_mode,smoothing,rolling,background_norm)["value"]
...
if overlay_mode.lower() == "confidence":
    ...
    # av_STARTS_long=av_STARTS.reset_index().melt(id_vars=["position", "sample_nr"],  value_name="score", var_name="phenotype").sort_values(by="position")
    ...
    plt.close()
    # Fig_STARTS = sns.lineplot(data=av_STARTS_long, x="position", y="score", hue="phenotype",)
    # plt.suptitle(f"adjusted read coverage: {target} target regions")
    # plt.xlabel("Position relative to target site")
    # plt.ylabel("normalized read ends coverage")
    # plt.close()
    ...
    plt.ylabel("normalized read coverage")
    plt.close()
    # Fig_STARTS = sns.lineplot(data=av_STARTS)
    # plt.suptitle(f"adjusted read end coverage: {target} target regions")
    # plt.xlabel("Position relative to target site")
    # plt.ylabel("normalized read end coverage")
    # plt.close()

with PdfPages(outfile) as pdf:
    pdf.savefig(Fig_WPS.get_figure())
    pdf.savefig(Fig_Cov.get_figure())
    #pdf.savefig(Fig_STARTS.get_figure())
